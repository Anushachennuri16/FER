...
# load json and create mode
    json_file = open('model/best_emodel.json', 'r')
    ...
    for (x, y, w, h) in faces:
        cv2.rectangle(img1, (x, y-50), (x+w, y+h+10), (0, 255, 0), 4)
        roi_gray_frame = gray[y:y + h, x:x + w]
        
        cropped_img = np.expand_dims(np.expand_dims(
            cv2.resize(roi_gray_frame, (96, 96)), -1), 0)

        # predict the emotions
        emotion_prediction = emotion_model.predict(cropped_img)
        maxindex = int(np.argmax(emotion_prediction))
        imk= cv2.putText(img1, emotion_dict[maxindex], (x+5, y-20),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        cv2.imwrite('static/after.jpg', imk)
    ...
    final_prediction=emotion_dict[maxindex]
...
# load weights into new model
    emotion_model.load_weights("model/best_model.h5")
